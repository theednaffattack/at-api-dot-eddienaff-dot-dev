...
const main = async () => {
  ...
  while (retries) {
    try {
      ...
      console.log(
        inspect(
          { ormConnection, env: nodeEnvIsProd ? "prod" : "not-prod" },
          false,
          3,
          true,
        ),
  ...
  const migrations: Migration[] | undefined = myPostgresConnection
    ? await myPostgresConnection.runMigrations()
    : undefined;

  console.log("VIEW MIGRATIONS", inspect({ migrations }, false, 3, true));
  // console.log(
  //   "VIEW POSTGRES / TYPEORM CONNECTION INFORMATION",
  //   inspect(myPostgresConnection, false, 3, true),
  // );

  // console.log(
  //   "VIEW POSTGRES / TYPEORM CONNECTION INFORMATION",
  //   inspect({ env: nodeEnvIsProd ? "prod" : "not-prod" }, false, 3, true),
  // );
    ...
    origin: function(origin: any, callback: any) {
      ...
      if (!origin || approvedOrigins.indexOf(origin) !== -1) {
        ...
        console.error(`cors error:: origin: `, {
          ...
          hostUsed,
          portUsed,
          lanIp: `http://${homeIp ? homeIp : hostUsed}:${
            port ? port : portUsed
          }`,
